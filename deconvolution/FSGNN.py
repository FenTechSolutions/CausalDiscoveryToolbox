...
def eval_feature_selection_score(df_data, target, verbose=False):
    ...
    G_dist_loss = MMD_loss(all_real_variables, all_generated_variables)
    ...
    for it in range(SETTINGS.nb_epoch_train_feature_selection):
        ...
        _, G_dist_loss_curr, complexity_curr, W_in_curr = sess.run([G_solver, G_dist_loss,model_complexity, W_in],feed_dict={all_parent_variables: data_features, target_variable: data_target})
...
class FSGNN(DeconvolutionModel):
    ...
    def create_skeleton_from_data(self, data):
        ...
        for _ in range(SETTINGS.nb_run_feature_selection):
            ...
            result_feature_selection = Parallel(n_jobs=SETTINGS.nb_jobs)(delayed(run_feature_selection)(data,idx, node) for idx,node in enumerate(list_nodes))
