...
p_directed_graph = GNN.orient_graph_confounders(
    data, skeleton, printout=datafile + '_printout.csv')

gnn_res = pd.DataFrame(p_directed_graph.list_edges(
    descending=True), columns=['Cause', 'Effect', 'Score'])
...
cgnn_res = pd.DataFrame(directed_graph.list_edges(
    descending=True), columns=['Cause', 'Effect', 'Score'])
